...
{
	ctx.cpuState.CPUControl[C0_CAUSE]._u32 &= ~mask;
	ctx.cpuState.CPUControl[C0_CAUSE]._u32 |= exception;
}
...
inline void R4300_JumpToInterruptVector(u32 exception_vector)
{
	...
	ctx.cpuState.CPUControl[C0_EPC]._u32  = ctx.cpuState.CurrentPC;


	if(ctx.cpuState.Delay == EXEC_DELAY)
	{
		ctx.cpuState.CPUControl[C0_CAUSE]._u32 |= CAUSE_BD;
		ctx.cpuState.CPUControl[C0_EPC]._u32   -= 4;
	}
	else
	{
		ctx.cpuState.CPUControl[C0_CAUSE]._u32 &= ~CAUSE_BD;
...
void R4300_Exception_Break()
{
	...
	gExceptionPC = ctx.cpuState.CurrentPC;
	gExceptionWasDelay = ctx.cpuState.Delay == EXEC_DELAY;
	ctx.cpuState.AddJob( CPU_CHECK_EXCEPTIONS );
}
...
void R4300_Exception_CopUnusuable()
{
    ...
    //ctx.cpuState.CPUControl[C0_CAUSE]._u32 &= 0xCFFFFFFF;
	//ctx.cpuState.CPUControl[C0_CAUSE]._u32 |= SR_CU0;
...
void R4300_Exception_TLB( u32 virtual_address, u32 exception_code, u32 exception_vector )
{
	...
	DAEDALUS_ASSERT( gExceptionPC == u32(~0), "Exception PC already set" );
	#endif
	ctx.cpuState.CPUControl[C0_BADVADDR]._u32 = virtual_address;

	ctx.cpuState.CPUControl[C0_CONTEXT]._u32 &= 0xFF800000;	// Mask off bottom 23 bits
	ctx.cpuState.CPUControl[C0_CONTEXT]._u32 |= ((virtual_address >> 13) << 4);

	ctx.cpuState.CPUControl[C0_ENTRYHI]._u32 &= 0x00001FFF;	// Mask off the top bit 13-31
	ctx.cpuState.CPUControl[C0_ENTRYHI]._u32 |= (virtual_address & 0xFFFFE000);
...
void R4300_Handle_Exception()
{
	...
	#ifdef DAEDALUS_ENABLE_ASSERTS
	DAEDALUS_ASSERT( gExceptionVector != u32(~0), "Exception vector not set: %08x", ctx.cpuState.GetStuffToDo() );
	...
	ctx.cpuState.CurrentPC = gExceptionPC;									// Restore this...
	ctx.cpuState.Delay = gExceptionWasDelay ? EXEC_DELAY : NO_DELAY;	// And this...
...
void R4300_Handle_Interrupt()
{
	...
	if( (ctx.cpuState.CPUControl[C0_SR]._u32 & (SR_EXL | SR_ERL | SR_IE)) == SR_IE ) // Ensure ERL/EXL are "0" and IE is "1"
	{
		if(ctx.cpuState.CPUControl[C0_SR]._u32 & ctx.cpuState.CPUControl[C0_CAUSE]._u32 & CAUSE_IPMASK)  // Are interrupts pending/wanted?
		{
#ifdef DAEDALUS_PROFILE_EXECUTION
			gNumInterrupts++;
#endif
