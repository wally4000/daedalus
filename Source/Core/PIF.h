...
class CController 
{
	public:
		...
		static bool Reset() { return Get().OnRomOpen(); }
		static void RomClose() { Get().OnRomClose(); }
	...
	private:
		...
		enum
		{
			CONT_TX_SIZE_CHANSKIP   = 0x00,
			CONT_TX_SIZE_DUMMYDATA  = 0xFF,
			CONT_TX_SIZE_FORMAT_END = 0xFE,
			CONT_TX_SIZE_CHANRESET  = 0xFD,
		};
