...
#endif
{
	...
	if( !ctx.videoMemoryManager->Alloc( bytes_required, &mpData, &mIsDataVidMem ) )
	{
			#ifdef DAEDALUS_DEBUG_CONSOLE
	...
	case TexFmt_CI4_8888:
		if( !ctx.videoMemoryManager->Alloc( kPalette4BytesRequired, &mpPalette, &mIsPaletteVidMem ) )
		{
				#ifdef DAEDALUS_DEBUG_CONSOLE
	...
	case TexFmt_CI8_8888:
		if( !ctx.videoMemoryManager->Alloc( kPalette8BytesRequired, &mpPalette, &mIsPaletteVidMem ) )
		{
			#ifdef DAEDALUS_DEBUG_CONSOLE
...
{
	ctx.videoMemoryManager->Free( mpData );
	ctx.videoMemoryManager->Free( mpPalette );
}
