...
#endif
{
	...
	if( !gVideo_MemoryManager->Alloc( bytes_required, &mpData, &mIsDataVidMem ) )
	{
			#ifdef DAEDALUS_DEBUG_CONSOLE
	...
	case TexFmt_CI4_8888:
		if( !gVideo_MemoryManager->Alloc( kPalette4BytesRequired, &mpPalette, &mIsPaletteVidMem ) )
		{
				#ifdef DAEDALUS_DEBUG_CONSOLE
	...
	case TexFmt_CI8_8888:
		if( !gVideo_MemoryManager->Alloc( kPalette8BytesRequired, &mpPalette, &mIsPaletteVidMem ) )
		{
			#ifdef DAEDALUS_DEBUG_CONSOLE
...
{
	gVideo_MemoryManager->Free( mpData );
	gVideo_MemoryManager->Free( mpPalette );
}
